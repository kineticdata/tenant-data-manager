  ...
  module Platform
    class Install < Kinetic::Platform::ActionBase
      ...
      def execute
        ...
        Kinetic::Platform.logger.info "Checking if the #{@core.space_slug} space slug is already installed"
        ...
        if res.status == 404
          ...
          msg = "Aborting #{ACTION}, a space with the slug #{@core.space_slug} is already installed."
          Kinetic::Platform.logger.info msg
        else
          msg = "#{res.status}: Aborting #{ACTION} of space slug #{@core.space_slug}, #{res.message}"
          Kinetic::Platform.logger.info msg
        end
      end
