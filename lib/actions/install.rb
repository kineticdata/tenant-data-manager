    ...
    class Install < Kinetic::Platform::ActionBase
      ...
      def execute
        begin
          ...
          # set the service user password that will be used by the
          ...
          Kinetic::Platform.logger.info "Creating user #{@core.service_user_username}"
          http = Http.new(@core.username, @core.password, @http_options)
          payload = {
            "space_slug" => @core.space_slug,
            "username" => @core.service_user_username,
            "password" => @core.service_user_password,
            "enabled" => true,
            "spaceAdmin" => true
          }
          url = "#{@core.system_api}/spaces/#{@core.space_slug}/users"
          ...
          if res.status == 200
            ...
            if task_is_running
              ...
              payload = {
                ...
                "properties" => {
                  "Kinetic Core Space Url" => "#{@core.server}",
                  "Proxy Username (Space Admin)" => @core.service_user_username,
                  "Proxy Password (Space Admin)" => @core.service_user_password
                }
              }
